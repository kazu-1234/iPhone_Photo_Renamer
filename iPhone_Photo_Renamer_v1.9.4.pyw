...
# =============================================================================
APP_VERSION = "1.9.4"
APP_TITLE = f"iPhone写真・動画リネーマー v{APP_VERSION}"

# GitHubリポジトリ情報 (EXE化の前に修正してください)
# 現在はプレースホルダーです
GITHUB_USER = "kazu-1234" 
GITHUB_REPO = "iPhone_Photo_Renamer"
...
class ImageRenamerApp(tk.Tk):
    ...
    v1.9.4:
      - UIをタブ形式に変更
      - 設定タブの追加（バージョン情報、アップデート確認、免責事項）
    v1.9.3: 
      - プログレスバーの進捗率表示の不具合を修正
    """
    ...
    def _setup_main_tab_content(self):
        ...
        # 1. ファイル選択フレーム
        selection_frame = ttk.LabelFrame(parent, text="ステップ1: 対象の選択", padding="10")
        ...
        self.file_list_label = ttk.Label(parent, text="対象ファイル: 0個")
        self.file_list_label.pack(anchor=tk.W, pady=5)

        # 1.5. ファイル種類選択フレーム
        file_type_frame = ttk.LabelFrame(parent, text="ステップ1.5: 対象ファイルの種類", padding="10")
        ...
        # 2. リネーム設定フレーム
        settings_frame = ttk.LabelFrame(parent, text="ステップ2: ファイル名の設定", padding="10")
        ...
        # 2.5 クリーンアップ・変換設定
        cleanup_frame = ttk.LabelFrame(parent, text="ステップ2.5: クリーンアップ・変換設定", padding="10")
        ...
        # 3. 実行ボタン
        self.rename_button = ttk.Button(parent, text="名前を変更して新しいフォルダにコピー", command=self.start_rename_process)
        self.rename_button.pack(fill=tk.X, pady=10)
        
        # 4. 進捗表示フレーム
        self.progress_label_var = tk.StringVar()
        progress_label = ttk.Label(parent, textvariable=self.progress_label_var)
    ...
    def _setup_settings_tab_content(self):
        ...
        disclaimer_text = (
            ...
            "GitHubのリポジトリにてソースコードを公開しており、"
            "必要に応じて内容を確認・修正して利用することができます。"
        )
        
        text_widget = tk.Text(disclaimer_frame, height=10, wrap="word", padx=5, pady=5, bg="#F0F0F0", relief="flat")
    ...
    def _update_check_worker(self):
        try:
            ...
            req.add_header('User-Agent', 'iPhonePhotoRenamer')
    ...
    def run_rename_thread(self, dest_folder, prefix, padding, start_num, convert_heic):
        ...
        self.safe_set_progress_max(total_files)
        ...
        self.safe_set_progress_max(total_to_copy)
        copied_count = 0
